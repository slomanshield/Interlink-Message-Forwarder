...
int Tier2::StopProcessing(bool forceStop)
{
	...
	cc = pQueueManager->WaitForQueueToDrain<MESSAGE>(&input_tier2_msg_queue, dbWaitTimeout);/* wait for queue to drain */

	if (cc != QUEUE_SUCCESS)
	{
		size_t queueSize = 0;
		pQueueManager->GetQueuesize<MESSAGE>(&input_tier2_msg_queue, &queueSize);
